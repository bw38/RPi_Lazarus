...
begin
  ...
  if FileExistsUTF8(fn) { *Konvertiert von FileExists* } then DL1.LoadFromFile(fn);
  Changed:= false;
...
begin
  if Changed then begin
    if not DirectoryExistsUTF8(Path) { *Konvertiert von DirectoryExists* } then ForceDirectoriesUTF8(Path); { *Konvertiert von ForceDirectories* }
    //Backupbeahndlung !!!!!!!!!
